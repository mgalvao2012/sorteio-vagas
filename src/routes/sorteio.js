			...
			unidades.forEach((unidade) => {
				...
				if (vagas_escolhidas) {
					for (let i = 0; i < vagas_escolhidas.length; i++) {
						...
						let vaga_index = vagas_disponiveis.indexOf(vagas_escolhidas[i].vaga);
					...
					if (error) {
						...
						if (results[0].rows[0] == undefined) {
							...
							response.render("sorteio.ejs", {
								...
								resultado_bloqueio: results[0].rows[0].resultado_bloqueio,
                          ...
                          vaga sorteada: ${element[1]} ordem_vaga_escolhida: ${element[2]}<br/>`;
                              ...
                              vaga sorteada: ${element[1]} ordem_vaga_escolhida: ${element[2]}<br/>`;
                                ...
                                resultado_sorteio = '${mensagem}' WHERE id = 1; `;
														...
														query_gravacao += `UPDATE configuracao SET ultimo_sorteio = '${data_atual}', 
                                					resultado_sorteio = '${mensagem}', 
															log_sorteio = '${query_gravacao_log_sorteio}' WHERE id = 1; `;
														pool.query(
															query_gravacao,
															(_error, _results) => {
																if (_error) {
								...
								.catch((error2) => {
									console.log(error2);
									response.status(500).json({ status: "error", message: error2 });
								});
						})
						.catch((error1) => {
							console.log(error1);
			...
			(error, results) => {
				if (error) {
					...
					if (results[1].rows[0].resultado_sorteio == "Sorteio não realizado") {
						...
						response.status(500).json({
							status: "error",
							message: "Falha no processo de reinicio. Procure o administrador.",
						});
					}
				}
			}
			...
			(error, results) => {
				if (error) {
					...
					if (results.rows[0].resultado_bloqueio == "Sorteio bloqueado") {
						...
						request.session.sorteioMensagem = "Sorteio bloqueado com sucesso!";
						getSorteio(request, response);
					} else {
						response.status(500).json({
							status: "error",
							message: "Falha no processo de bloqueio. Procure o administrador.",
						});
					}
				}
			}
